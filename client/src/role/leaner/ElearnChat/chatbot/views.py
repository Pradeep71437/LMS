...
def ensure_uploads_file():
    """Ensure the uploads Excel file exists with required columns"""
...
def videos_view(request):
    ...
    if request.method == 'POST':
        if not GLOBAL_ADMIN_LOGIN_STATUS:
    ...
    # Render the page and pass whether the user is a superuser to the template
    return render(request, 'videos.html', {'videos': videos, 'is_superuser': request.user.is_superuser})
